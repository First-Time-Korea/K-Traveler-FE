...
const emit = defineEmits(["clickRegionEvent"]);
...
const clickRegionEvent = (region) => {
  emit("clickRegionEvent", region);
};
...
watch(
  ...
  (newKeyword) => {
    filteredRegions.value = regions.value
      .filter((region) => region.sidoName.toLowerCase().includes(newKeyword.toLowerCase()))
      .slice(0, 10);
  }
);
</script>
