...
function addToPlan(dateString, attraction) {
  if (travelPlans.value[dateString] !== undefined) {
    ...
    if (
      travelPlans.value[dateString].some(
        (attraction) => attraction.contentId === attractionContentId
      )
    ) {
      return;
    }
...
function deleteAtPlan(dateString, attrraction) {
  const deletedPlans = travelPlans.value[dateString].filter(
    (attr) => attr.contentId != attrraction.contentId
  );
...
function onSelectChange() {
  ...
  getAttractionBySidoCode(
    ...
    ({ data }) => {
      ...
      otherParam.value.sidocode = sidoCode;

      otherCurrentPage.value = 1;
      otherTotalPage.value = data.totalPageCount;
    },
    (error) => {
      console.error(error);
    }
...
); //유저가 입력할 시간 300ms 주는 것

// 시도코드로 조회한 attraction 페이지네이션
...
const tryMoreGetAttractionsBySido = () => {
  currentPage.value += 1;
  ...
  getAttractionBySidoCode(
    param.value,
    ({ data }) => {
      filteredAttractionsToShow.value = [...filteredAttractionsToShow.value, ...data.data];
    },
    (error) => {
      console.error(error);
    }
...
const canMoreGetAttractionsBySidoCode = computed(() => {
  return currentTab.value === "selected regions" && currentPage.value < totalPage.value;
});

const otherCurrentPage = ref(1);
const otherTotalPage = ref(0);
const otherParam = ref({
  pgno: otherCurrentPage.value,
  sidocode: 0,
});

const tryMoreGetAttractionsByOtherSido = () => {
  otherCurrentPage.value += 1;
  otherParam.value.pgno = otherCurrentPage.value;
...
const canMoreGetAttractionsByOtherSidoCode = computed(() => {
  return currentTab.value === "other regions" && otherCurrentPage.value < otherTotalPage.value;
});
</script>
    ...
    <div class="flex flex-1 justify-center">
      <nav class="text-sm w-96">
        <div
          ...
          :key="tab"
          :class="{
            'border-b-2 border-first-500 text-first-500 font-semibold': currentTab === tab,
            'text-gray-700 hover:text-black': currentTab !== tab,
          }"
        ...
        <div class="scrollable-container custom-scroll">
          <div
            class="flex bg-white hover:shadow-lg hover:opacity-75 focus:ring focus:ring-gray-300 mb-2 p-2 rounded-lg relative transition-shadow"
            v-for="attraction in filteredAttractionsToShow"
            :key="attraction.id"
          >
            <div class="w-24 h-48 overflow-hidden">
              <img
                :src="attraction.firstImage ? attraction.firstImage : defaultImg"
                alt=""
            ...
            <button
              v-if="isInPlan(selectedDate, attraction.contentId)"
          ...
          <!-- 여행지 더 불러오기 -->
          <div class="flex justify-center mt-10" v-show="canMoreGetAttractionsBySidoCode">
            <button
              ...
              type="button"
              v-show="canMoreGetAttractionsBySidoCode"
              @click="tryMoreGetAttractionsBySido"
            >
              MORE
            </button>
          </div>
          <div class="flex justify-center mt-10" v-show="canMoreGetAttractionsByOtherSidoCode">
            <button
              class="px-6 py-3 font-sans text-xs font-bold text-center text-first-400 uppercase align-middle transition-all rounded-lg select-none hover:bg-first-300/10 active:bg-first-300/20 disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none"
              type="button"
              v-show="canMoreGetAttractionsByOtherSidoCode"
              @click="tryMoreGetAttractionsByOtherSido"
            >
              MORE
            </button>
          </div>
        ...
        <div class="selected-container scroll">
          <div v-if="travelPlans[selectedDate] && travelPlans[selectedDate].length > 0">
            <div v-for="(attraction, index) in travelPlans[selectedDate]" :key="index">
              <button
                class="p-2 rounded-full cursor-pointer absolute right-2"
